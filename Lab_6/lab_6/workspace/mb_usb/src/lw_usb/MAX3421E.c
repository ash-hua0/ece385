...
void SPI_init() {
	...
	xil_printf("%d\n", SpiInstance.SlaveSelectReg);

	ConfigPtr = XSpi_LookupConfig(XPAR_SPI_USB_DEVICE_ID); //Pointer to some of the device info
	if (ConfigPtr == NULL) {
		return XST_DEVICE_NOT_FOUND;
	}

	Status = XSpi_CfgInitialize(&SpiInstance, ConfigPtr, //Did we start the SPI device successfully?
	...
	XSpi_IntrGlobalDisable(&SpiInstance); //Puts driver into polling mode
}
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	u8 ReadBuf[2] = {reg + 2, val};
	u8 WriteBuf[2] = {0, 0};
	int t = XSpi_Transfer(&SpiInstance, WriteBuf, ReadBuf, 2);
	xil_printf("%d\n", t);
...
void MAX3421E_init(void) {
	...
	for (int i = 0; i < 3; i++)
	{
		...
		while (XTmrCtr_GetValue(&Usb_timer, 0) - current < 100000000) //Uses timer module we put in block diagram
		{
