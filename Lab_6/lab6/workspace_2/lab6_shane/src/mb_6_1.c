...
volatile uint32_t* hex_gpio_data = (uint32_t*)0x40030000;
volatile uint32_t* accumulate = (uint32_t*)0x40020000;
...
int main()
{
    ...
    while (1){
    	...
    	*hex_gpio_data = val;
    	...
    	if (*accumulate == 0 && prev == 1){ //negedge accumulate (acc 0, prev 1)
    		if (val + *sw_gpio_data <= 65536){
				val = val + *sw_gpio_data;
				//printf("update");
			}
			else{
				//printf("%lu", *accumulate);
