...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	hdmi_ctrl->VRAM[0x12BD + color*4] = (red << 8) + (green << 4) + blue;
}
...
hdmiTestWeek2()
{
	...
	for (int j = 0; j < ROWS; j++)
	{
		...
		for (int i = 0; i < COLUMNS * 2; i++)
		{
			printf("%d, ", hdmi_ctrl->VRAM[j*COLUMNS*2 + i]);
	...
	hdmi_ctrl->VRAM[0x4B0] = 0x4 << 4;
	printf("\n\ncolor palette: %x, ", hdmi_ctrl->VRAM[0x4B0]);
	printf("\n\ncolor palette: %x, ", hdmi_ctrl->VRAM[0x4B0]);
	setColorPalette(1, 	0xA, 0xB, 0xA);
	printf("\n\ncolor palette: %d, ", hdmi_ctrl->VRAM[0x4B1]);


	printf ("Checksum passed, beginning palette test\n\r");

	paletteTest();
	printf("\n\ncolor palette 0: %d, ", hdmi_ctrl->VRAM[0x4B0]);
	printf("\n\ncolor palette 1: %d, ", hdmi_ctrl->VRAM[0x4B1]);
	printf("\n\ncolor palette 2: %d, ", hdmi_ctrl->VRAM[0x4B2]);
	printf("\n\ncolor palette 0: %d, ", hdmi_ctrl->VRAM[0x4B0]);
	printf("\n\ncolor palette 0: %d, ", hdmi_ctrl->VRAM[0x4B0]);
